...
function rewrite_url()
    if config_url_check == "on" then
        local REWRITE_RULES = get_rule('rewrite.rule')
        local REQ_URL = ngx.var.host
        local REQ_SCHEME = ngx.var.scheme
        for _,rule in pairs(REWRITE_RULES) do
            if rule ~= "" and rulematch(REQ_URL,rule,"jo") and REQ_SCHEME == "http" then
                return ngx.redirect('https://'..REQ_URL)
            end
        end
    end
    return false
    ...
    if config_post_check == "on" then
	    ...
	    ngx.req.read_body()
            ...
            for key, val in pairs(POST_ARGS) do
                ...
                if ARGS_DATA and rule ~="" and rulematch(unescape(ARGS_DATA),rule,"jo") then
